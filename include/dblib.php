 ...
 * $dbuser and $dbpass. */
	...
	if (! $dbh = @mysql_pconnect($dbhost, $dbuser, $dbpass)) {
		...
		if ($DB_DEBUG) {
			echo "<li><strong>".ucwords($CONFIG['dbsystem'])." DB Error 1 </strong>: ". mysql_error();
		}
	...
	if (!@mysql_select_db($dbname)) {
		echo "<h3>Database error encountered</h3>";
		if ($DB_DEBUG) {
			echo "<li><strong>".ucwords($CONFIG['dbsystem'])." DB Error 2</strong>: ". mysql_error();
		}
 ...
 * occurred */
	...
	if (! $qid && ! $silent) {
		echo "<h3>Database error encountered</h3>";
		if ($DB_DEBUG) {
			echo "<li><strong>".ucwords($CONFIG['dbsystem'])." DB Error 3</strong>: ". mysql_error();
		}
